...
from classes.CustomNetwork import CustomNetwork
...
n_classes = 10
assignments = -torch.ones(n_output, device=device)
proportions = torch.zeros((n_output, n_classes), device=device)
rates = torch.zeros((n_output, n_classes), device=device)
...
spike_record = torch.zeros((update_interval, int(time / dt), n_output), device=device)
