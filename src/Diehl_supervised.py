...
from classes.DiehlAndCook2015_supervised import DiehlAndCook2015
...
parser.add_argument("--test_update_interval", type=int, default=10)
...
test_update_interval = args.test_update_interval
train = args.train
plot = args.plot
gpu = args.gpu
...
for step, batch in enumerate(test_dataset):
    ...
    if step % (test_update_interval * 10) == 0 and step > 0:
        #pbar.set_description_str("Testing progress:")
        pbar.update()
        print(f"\nAccuracy: {(max(accuracy['all'] ,accuracy['proportion'] ) / (step+1)):.3}")
